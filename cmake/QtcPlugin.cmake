...
function(qtc_generate_pluginspec target)
    ...
    set(txtCmakeLists ${CMAKE_CURRENT_SOURCE_DIR}/CMakeLists.txt)
    if(${txtCmakeLists} IS_NEWER_THAN ${specFile}
        OR ${specInFile} IS_NEWER_THAN ${specFile}
        OR VERSION_UPDATED)
        file(READ ${specInFile} specInContent)
        set(dependency "<dependencyList>")
        foreach(d ${depends})
            set(dependency "${dependency}\n        <dependency name=\"${d}\" version=\"${QTCREATOR_VERSION}\"/>")
        endforeach()
        set(dependency "${dependency}\n    </dependencyList>")
        string(REPLACE $$QTCREATOR_VERSION ${QTCREATOR_VERSION} specInContent "${specInContent}")
        string(REPLACE $$dependencyList "${dependency}" specInContent "${specInContent}")
